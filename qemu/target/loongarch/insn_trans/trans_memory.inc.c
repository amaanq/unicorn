...
static bool gen_load(DisasContext *ctx, arg_rr_i *a, MemOp mop)
{
    ...
    tcg_gen_qemu_ld_tl(tcg_ctx, dest, addr, ctx->mem_idx, mop);
...
static bool gen_store(DisasContext *ctx, arg_rr_i *a, MemOp mop)
{
    ...
    tcg_gen_qemu_st_tl(tcg_ctx, data, addr, ctx->mem_idx, mop);
...
static bool gen_load_gt(DisasContext *ctx, arg_rrr *a, MemOp mop)
{
    ...
    tcg_gen_qemu_ld_tl(tcg_ctx, dest, src1, ctx->mem_idx, mop);
    gen_set_gpr(tcg_ctx, a->rd, dest, EXT_NONE);
...
static bool gen_store_gt(DisasContext *ctx, arg_rrr *a, MemOp mop)
{
    ...
    gen_helper_asrtgt_d(tcg_ctx, tcg_ctx->cpu_env, src1, src2);
    tcg_gen_qemu_st_tl(tcg_ctx, data, src1, ctx->mem_idx, mop);
...
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;
    tcg_gen_mb(tcg_ctx, TCG_BAR_SC | TCG_MO_ALL);
    return true;
}
