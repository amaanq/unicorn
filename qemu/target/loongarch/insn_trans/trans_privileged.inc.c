...
typedef void (*GenCSRRead)(TCGContext *tcg_ctx, TCGv dest, TCGv_ptr env);
typedef void (*GenCSRWrite)(TCGContext *tcg_ctx, TCGv dest, TCGv_ptr env, TCGv src);
...
static bool trans_csrrd(DisasContext *ctx, arg_csrrd *a)
{
    ...
    if (csr == NULL) {
        ...
        dest = tcg_constant_tl(tcg_ctx, 0);
    } else {
        check_csr_flags(ctx, csr, false);
        dest = gpr_dst(ctx, a->rd, EXT_NONE);
        if (csr->readfn) {
            csr->readfn(tcg_ctx, dest, tcg_ctx->cpu_env);
        } else {
            tcg_gen_ld_tl(tcg_ctx, dest, tcg_ctx->cpu_env, csr->offset);
        }
    }
...
static bool trans_csrwr(DisasContext *ctx, arg_csrwr *a)
{
    ...
    if (csr == NULL) {
        ...
        gen_set_gpr(tcg_ctx, a->rd, tcg_constant_tl(tcg_ctx, 0), EXT_NONE);
        return true;
    }
    ...
    if (csr->writefn) {
        dest = gpr_dst(ctx, a->rd, EXT_NONE);
        csr->writefn(tcg_ctx, dest, tcg_ctx->cpu_env, src1);
    } else {
        dest = temp_new(ctx);
        tcg_gen_ld_tl(tcg_ctx, dest, tcg_ctx->cpu_env, csr->offset);
        tcg_gen_st_tl(tcg_ctx, src1, tcg_ctx->cpu_env, csr->offset);
    }
    gen_set_gpr(tcg_ctx, a->rd, dest, EXT_NONE);
    return true;
}
...
static bool trans_csrxchg(DisasContext *ctx, arg_csrxchg *a)
{
    ...
    tcg_gen_ld_tl(tcg_ctx, oldv, tcg_ctx->cpu_env, csr->offset);
    tcg_gen_and_tl(tcg_ctx, newv, src1, mask);
    tcg_gen_andc_tl(tcg_ctx, temp, oldv, mask);
    tcg_gen_or_tl(tcg_ctx, newv, newv, temp);

    if (csr->writefn) {
        csr->writefn(tcg_ctx, oldv, tcg_ctx->cpu_env, newv);
    } else {
        tcg_gen_st_tl(tcg_ctx, newv, tcg_ctx->cpu_env, csr->offset);
    }
    gen_set_gpr(tcg_ctx, a->rd, oldv, EXT_NONE);
...
static bool gen_iocsrrd(DisasContext *ctx, arg_rr *a,
                        void (*func)(TCGContext *, TCGv, TCGv_ptr, TCGv))
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;
    ...
    func(tcg_ctx, dest, tcg_ctx->cpu_env, src1);
    return true;
}

static bool gen_iocsrwr(DisasContext *ctx, arg_rr *a,
                        void (*func)(TCGContext *, TCGv_ptr, TCGv, TCGv))
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;
    ...
    func(tcg_ctx, tcg_ctx->cpu_env, addr, val);
    return true;
}
...
static void check_mmu_idx(DisasContext *ctx)
{
    ...
    if (ctx->mem_idx != MMU_DA_IDX) {
        tcg_gen_movi_tl(tcg_ctx, tcg_ctx->cpu_pc, ctx->base.pc_next + 4);
...
static bool trans_tlbsrch(DisasContext *ctx, arg_tlbsrch *a)
{
    ...
    gen_helper_tlbsrch(tcg_ctx, tcg_ctx->cpu_env);
    return true;
}
...
static bool trans_tlbrd(DisasContext *ctx, arg_tlbrd *a)
{
    ...
    gen_helper_tlbrd(tcg_ctx, tcg_ctx->cpu_env);
    return true;
}
...
static bool trans_tlbfill(DisasContext *ctx, arg_tlbfill *a)
{
    ...
    gen_helper_tlbfill(tcg_ctx, tcg_ctx->cpu_env);
    check_mmu_idx(ctx);
    return true;
}
...
static bool trans_tlbclr(DisasContext *ctx, arg_tlbclr *a)
{
    ...
    gen_helper_tlbclr(tcg_ctx, tcg_ctx->cpu_env);
    check_mmu_idx(ctx);
    return true;
}
...
static bool trans_tlbflush(DisasContext *ctx, arg_tlbflush *a)
{
    ...
    gen_helper_tlbflush(tcg_ctx, tcg_ctx->cpu_env);
    check_mmu_idx(ctx);
    return true;
}
...
static bool trans_invtlb(DisasContext *ctx, arg_invtlb *a)
{
    ...
    case 0:
    case 1:
        gen_helper_invtlb_all(tcg_ctx, tcg_ctx->cpu_env);
        break;
    case 2:
        gen_helper_invtlb_all_g(tcg_ctx, tcg_ctx->cpu_env, tcg_constant_i32(tcg_ctx, 1));
        break;
    case 3:
        gen_helper_invtlb_all_g(tcg_ctx, tcg_ctx->cpu_env, tcg_constant_i32(tcg_ctx, 0));
        break;
    case 4:
        gen_helper_invtlb_all_asid(tcg_ctx, tcg_ctx->cpu_env, rj);
        break;
    case 5:
        gen_helper_invtlb_page_asid(tcg_ctx, tcg_ctx->cpu_env, rj, rk);
        break;
    case 6:
        gen_helper_invtlb_page_asid_or_g(tcg_ctx, tcg_ctx->cpu_env, rj, rk);
        break;
    default:
        return false;
    }
...
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;
    TCGv_i32 mem_idx = tcg_constant_i32(tcg_ctx, ctx->mem_idx);
    TCGv src1 = gpr_src(ctx, a->rj, EXT_NONE);

    if (check_plv(ctx)) {
        return false;
    }
    gen_helper_ldpte(tcg_ctx, tcg_ctx->cpu_env, src1, tcg_constant_tl(tcg_ctx, a->imm), mem_idx);
    return true;
}
...
static bool trans_lddir(DisasContext *ctx, arg_lddir *a)
{
    ...
    gen_helper_lddir(tcg_ctx, dest, tcg_ctx->cpu_env, src, tcg_constant_tl(tcg_ctx, a->imm), mem_idx);
    return true;
}
...
static bool trans_ertn(DisasContext *ctx, arg_ertn *a)
{
    ...
    gen_helper_ertn(tcg_ctx, tcg_ctx->cpu_env);
    ctx->base.is_jmp = DISAS_EXIT;
    return true;
}
...
static bool trans_idle(DisasContext *ctx, arg_idle *a)
{
    ...
    tcg_gen_movi_tl(tcg_ctx, tcg_ctx->cpu_pc, ctx->base.pc_next + 4);
    gen_helper_idle(tcg_ctx, tcg_ctx->cpu_env);
