                                ...
                                MMUAccessType access_type, int tlb_error)
{
    ...
    case TLBRET_NOMATCH:
        ...
        FIELD_DP64(env->CSR_TLBRERA, CSR_TLBRERA, ISTLBR, 1, env->CSR_TLBRERA);
        break;
    case TLBRET_INVALID:
    ...
    if (tlb_error == TLBRET_NOMATCH) {
        ...
        FIELD_DP64(env->CSR_TLBREHI, CSR_TLBREHI, VPPN,
                   extract64(address, 13, 35), env->CSR_TLBREHI);
    } else {
...
static void fill_tlb_entry(CPULoongArchState *env, int index)
{
    ...
    if (index >= LOONGARCH_STLB) {
        FIELD_DP64(tlb->tlb_misc, TLB_MISC, PS, csr_ps, tlb->tlb_misc);
    }

    FIELD_DP64(tlb->tlb_misc, TLB_MISC, VPPN, csr_vppn, tlb->tlb_misc);
    FIELD_DP64(tlb->tlb_misc, TLB_MISC, E, 1, tlb->tlb_misc);
    csr_asid = FIELD_EX64(env->CSR_ASID, CSR_ASID, ASID);
    FIELD_DP64(tlb->tlb_misc, TLB_MISC, ASID, csr_asid, tlb->tlb_misc);
...
void helper_tlbsrch(CPULoongArchState *env)
{
    ...
    if (match) {
        FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX, INDEX, index, env->CSR_TLBIDX);
        FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX, NE, 0, env->CSR_TLBIDX);
        return;
    }

    FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX, NE, 1, env->CSR_TLBIDX);
}
...
void helper_tlbrd(CPULoongArchState *env)
{
    ...
    if (!tlb_e) {
        /* Invalid TLB entry */
        FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX, NE, 1, env->CSR_TLBIDX);
        FIELD_DP64(env->CSR_ASID, CSR_ASID, ASID, 0, env->CSR_ASID);
        env->CSR_TLBEHI = 0;
        env->CSR_TLBELO0 = 0;
        env->CSR_TLBELO1 = 0;
        FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX, PS, 0, env->CSR_TLBIDX);
    } else {
        /* Valid TLB entry */
        FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX, NE, 0, env->CSR_TLBIDX);
        FIELD_DP64(env->CSR_TLBIDX, CSR_TLBIDX,
                   PS, (tlb_ps & 0x3f), env->CSR_TLBIDX);
...
void helper_tlbwr(CPULoongArchState *env)
{
    ...
    if (FIELD_EX64(env->CSR_TLBIDX, CSR_TLBIDX, NE)) {
        FIELD_DP64(env->tlb[index].tlb_misc,
                   TLB_MISC, E, 0, env->tlb[index].tlb_misc);
        return;
    }
    ...
    if (index < LOONGARCH_STLB) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (!tlb_g && tlb_asid == csr_asid) {
                FIELD_DP64(tlb->tlb_misc, TLB_MISC, E, 0, tlb->tlb_misc);
            }
        }
...
void helper_tlbflush(CPULoongArchState *env)
{
    ...
    if (index < LOONGARCH_STLB) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            FIELD_DP64(env->tlb[s_idx].tlb_misc,
                       TLB_MISC, E, 0, env->tlb[s_idx].tlb_misc);
        }
    } else if (index < LOONGARCH_TLB_MAX) {
        /* All MTLB entries */
        for (i = LOONGARCH_STLB; i < LOONGARCH_TLB_MAX; i++) {
            FIELD_DP64(env->tlb[i].tlb_misc,
                       TLB_MISC, E, 0, env->tlb[i].tlb_misc);
        }
    }
                          ...
                          target_ulong level, uint32_t mem_idx)
{
    ...
    ret = glue(ldq_phys, UNICORN_ARCH_POSTFIX)(cs->uc, cs->as, phys) & TARGET_PHYS_MASK;
    return ret;
}
                  ...
                  uint32_t mem_idx)
{
    ...
    if (huge) {
        ...
        tmp0 = glue(ldq_phys, UNICORN_ARCH_POSTFIX)(cs->uc, cs->as, phys) & TARGET_PHYS_MASK;
        ps = ptbase;
    }
    ...
    FIELD_DP64(env->CSR_TLBREHI, CSR_TLBREHI, PS, ps, env->CSR_TLBREHI);
}
