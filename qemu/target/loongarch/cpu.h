...
#define SET_FP_CAUSE(REG, V) \
    do { \
        FIELD_DP32(REG, FCSR0, CAUSE, V, (REG)); \
    } while (0)
...
#define UPDATE_FP_FLAGS(REG, V) \
    do { \
        unsigned int tmp; \
        FIELD_DP32(0, FCSR0, FLAGS, V, tmp); \
        (REG) |= tmp; \
    } while (0)
...
typedef struct CPULoongArchState {
    uint64_t gpr[32];
    uint64_t pc;
...
#define LOONGARCH_CPU_CLASS(klass) ((LoongArchCPUClass *)klass)
#define LOONGARCH_CPU_GET_CLASS(obj) (&((LoongArchCPU *)obj)->cc)
