...
#define DO_3SAME_BITSEL(INSN, O1, O2, O3)                               \
    static void gen_##INSN##_3s(TCGContext *tcg_ctx, unsigned vece, uint32_t rd_ofs,         \
...
#define DO_3SAME_64_ENV(INSN, FUNC)                                     \
    static void gen_##INSN##_elt(TCGContext *tcg_ctx, TCGv_i64 d, TCGv_i64 n, TCGv_i64 m)    \
    {                                                                   \
...
#define WRAP_ENV_FN(WRAPNAME, FUNC)                                     \
    static void WRAPNAME(TCGContext *tcg_ctx, TCGv_i32 d, TCGv_i32 n, TCGv_i32 m)            \
    {                                                                   \
        FUNC(tcg_ctx, d, tcg_ctx->cpu_env, n, m);                                         \
    }
...
static bool do_3same_pair(DisasContext *s, arg_3same *a, NeonGenTwoOpFn *fn)
{
    ...
    read_neon_element32(tcg_ctx, tmp, a->vn, 0, MO_32);
    read_neon_element32(tcg_ctx, tmp2, a->vn, 1, MO_32);
    fn(tcg_ctx, tmp, tmp, tmp2);

    read_neon_element32(tcg_ctx, tmp3, a->vm, 0, MO_32);
    read_neon_element32(tcg_ctx, tmp2, a->vm, 1, MO_32);
    fn(tcg_ctx, tmp3, tmp3, tmp2);

    write_neon_element32(tcg_ctx, tmp, a->vd, 0, MO_32);
    write_neon_element32(tcg_ctx, tmp3, a->vd, 1, MO_32);
...
#define WRAP_FP_GVEC(WRAPNAME, FPST, FUNC)                              \
    static void WRAPNAME(TCGContext *tcg_ctx, unsigned vece, uint32_t rd_ofs,                \
                         uint32_t rn_ofs, uint32_t rm_ofs,              \
                         uint32_t oprsz, uint32_t maxsz)                \
    {                                                                   \
        TCGv_ptr fpst = fpstatus_ptr(tcg_ctx, FPST);                             \
                             ...
                             NeonGenTwo64OpEnvFn *fn)
{
    ...
    for (pass = 0; pass < a->q + 1; pass++) {
        ...
        read_neon_element64(tcg_ctx, tmp, a->vm, pass, MO_64);
        fn(tcg_ctx, tmp, tcg_ctx->cpu_env, tmp, constimm);
        write_neon_element64(tcg_ctx, tmp, a->vd, pass, MO_64);
                             ...
                             NeonGenTwoOpEnvFn *fn)
{
    ...
    for (pass = 0; pass < (a->q ? 4 : 2); pass++) {
        read_neon_element32(tcg_ctx, tmp, a->vm, pass, MO_32);
        fn(tcg_ctx, tmp, tcg_ctx->cpu_env, tmp, constimm);
        write_neon_element32(tcg_ctx, tmp, a->vd, pass, MO_32);
    }
    tcg_temp_free_i32(tcg_ctx, tmp);
                                ...
                                NeonGenNarrowEnvFn *narrowfn)
{
    ...
    read_neon_element64(tcg_ctx, rm1, a->vm, 0, MO_64);
    read_neon_element64(tcg_ctx, rm2, a->vm, 1, MO_64);

    shiftfn(tcg_ctx, rm1, rm1, constimm);
    narrowfn(tcg_ctx, rd, tcg_ctx->cpu_env, rm1);
    write_neon_element32(tcg_ctx, rd, a->vd, 0, MO_32);

    shiftfn(tcg_ctx, rm2, rm2, constimm);
    narrowfn(tcg_ctx, rd, tcg_ctx->cpu_env, rm2);
    write_neon_element32(tcg_ctx, rd, a->vd, 1, MO_32);
                                ...
                                NeonGenNarrowEnvFn *narrowfn)
{
    ...
    read_neon_element32(tcg_ctx, rm1, a->vm, 0, MO_32);
    read_neon_element32(tcg_ctx, rm2, a->vm, 1, MO_32);
    read_neon_element32(tcg_ctx, rm3, a->vm, 2, MO_32);
    read_neon_element32(tcg_ctx, rm4, a->vm, 3, MO_32);
    ...
    write_neon_element32(tcg_ctx, rm1, a->vd, 0, MO_32);
    ...
    write_neon_element32(tcg_ctx, rm3, a->vd, 1, MO_32);
                         ...
                         NeonGenWidenFn *widenfn, bool u)
{
    ...
    rm1 = tcg_temp_new_i32(tcg_ctx);
    read_neon_element32(tcg_ctx, rm0, a->vm, 0, MO_32);
    read_neon_element32(tcg_ctx, rm1, a->vm, 1, MO_32);
    ...
    write_neon_element64(tcg_ctx, tmp, a->vd, 0, MO_64);
    ...
    write_neon_element64(tcg_ctx, tmp, a->vd, 1, MO_64);
    tcg_temp_free_i64(tcg_ctx, tmp);
    return true;
}
                           ...
                           int src1_mop, int src2_mop)
{
    ...
    if (src1_mop >= 0) {
        read_neon_element64(tcg_ctx, rn0_64, a->vn, 0, src1_mop);
    } else {
    ...
    if (src2_mop >= 0) {
        read_neon_element64(tcg_ctx, rm_64, a->vm, 0, src2_mop);
    } else {
        TCGv_i32 tmp = tcg_temp_new_i32(tcg_ctx);
        read_neon_element32(tcg_ctx, tmp, a->vm, 0, MO_32);
     ...
     */
    if (src1_mop >= 0) {
        read_neon_element64(tcg_ctx, rn1_64, a->vn, 1, src1_mop);
    } else {
        TCGv_i32 tmp = tcg_temp_new_i32(tcg_ctx);
        read_neon_element32(tcg_ctx, tmp, a->vn, 1, MO_32);
        widenfn(tcg_ctx, rn1_64, tmp);
        tcg_temp_free_i32(tcg_ctx, tmp);
    }
    if (src2_mop >= 0) {
        read_neon_element64(tcg_ctx, rm_64, a->vm, 1, src2_mop);
    } else {
        TCGv_i32 tmp = tcg_temp_new_i32(tcg_ctx);
        read_neon_element32(tcg_ctx, tmp, a->vm, 1, MO_32);
        widenfn(tcg_ctx, rm_64, tmp);
        tcg_temp_free_i32(tcg_ctx, tmp);
    }

    write_neon_element64(tcg_ctx, rn0_64, a->vd, 0, MO_64);

    opfn(tcg_ctx, rn1_64, rn1_64, rm_64);
    write_neon_element64(tcg_ctx, rn1_64, a->vd, 1, MO_64);
                         ...
                         NeonGenTwo64OpFn *opfn, NeonGenNarrowFn *narrowfn)
{
    ...
    read_neon_element64(tcg_ctx, rn_64, a->vn, 0, MO_64);
    read_neon_element64(tcg_ctx, rm_64, a->vm, 0, MO_64);

    opfn(tcg_ctx, rn_64, rn_64, rm_64);

    narrowfn(tcg_ctx, rd0, rn_64);

    read_neon_element64(tcg_ctx, rn_64, a->vn, 1, MO_64);
    read_neon_element64(tcg_ctx, rm_64, a->vm, 1, MO_64);

    opfn(tcg_ctx, rn_64, rn_64, rm_64);

    narrowfn(tcg_ctx, rd1, rn_64);

    write_neon_element32(tcg_ctx, rd0, a->vd, 0, MO_32);
    write_neon_element32(tcg_ctx, rd1, a->vd, 1, MO_32);
                       ...
                       NeonGenTwo64OpFn *accfn)
{
    ...
    read_neon_element32(tcg_ctx, rn, a->vn, 0, MO_32);
    read_neon_element32(tcg_ctx, rm, a->vm, 0, MO_32);
    opfn(tcg_ctx, rd0, rn, rm);

    read_neon_element32(tcg_ctx, rn, a->vn, 1, MO_32);
    read_neon_element32(tcg_ctx, rm, a->vm, 1, MO_32);
    ...
    if (accfn) {
        tmp = tcg_temp_new_i64(tcg_ctx);
        read_neon_element64(tcg_ctx, tmp, a->vd, 0, MO_64);
        accfn(tcg_ctx, rd0, tmp, rd0);
        read_neon_element64(tcg_ctx, tmp, a->vd, 1, MO_64);
        accfn(tcg_ctx, rd1, tmp, rd1);
        tcg_temp_free_i64(tcg_ctx, tmp);
    }

    write_neon_element64(tcg_ctx, rd0, a->vd, 0, MO_64);
    write_neon_element64(tcg_ctx, rd1, a->vd, 1, MO_64);
...
static inline TCGv_i32 neon_get_scalar(TCGContext *tcg_ctx, int size, int reg)
{
    TCGv_i32 tmp = tcg_temp_new_i32(tcg_ctx);
    if (size == MO_16) {
        read_neon_element32(tcg_ctx, tmp, reg & 7, reg >> 4, MO_32);
        if (reg & 8) {
        ...
        read_neon_element32(tcg_ctx, tmp, reg & 15, reg >> 4, MO_32);
    }
    return tmp;
}
                       ...
                       NeonGenTwoOpFn *opfn, NeonGenTwoOpFn *accfn)
{
    ...
    for (pass = 0; pass < (a->q ? 4 : 2); pass++) {
        read_neon_element32(tcg_ctx, tmp, a->vn, pass, MO_32);
        opfn(tcg_ctx, tmp, tmp, scalar);
        if (accfn) {
            TCGv_i32 rd = tcg_temp_new_i32(tcg_ctx);
            read_neon_element32(tcg_ctx, rd, a->vd, pass, MO_32);
                            ...
                            NeonGenThreeOpEnvFn *opfn)
{
    ...
    for (pass = 0; pass < (a->q ? 4 : 2); pass++) {
        read_neon_element32(tcg_ctx, rn, a->vn, pass, MO_32);
        read_neon_element32(tcg_ctx, rd, a->vd, pass, MO_32);
        opfn(tcg_ctx, rd, tcg_ctx->cpu_env, rn, scalar, rd);
        write_neon_element32(tcg_ctx, rd, a->vd, pass, MO_32);
    }
    tcg_temp_free_i32(tcg_ctx, rn);
    tcg_temp_free_i32(tcg_ctx, rd);
    tcg_temp_free_i32(tcg_ctx, scalar);
                            ...
                            NeonGenTwo64OpFn *accfn)
{
    ...
    if (accfn) {
        ...
        read_neon_element64(tcg_ctx, t64, a->vd, 0, MO_64);
        accfn(tcg_ctx, rn0_64, t64, rn0_64);
        read_neon_element64(tcg_ctx, t64, a->vd, 1, MO_64);
...
static bool trans_VEXT(DisasContext *s, arg_VEXT *a)
{
    ...
    if (!a->q) {
        ...
        read_neon_element64(tcg_ctx, right, a->vn, 0, MO_64);
        read_neon_element64(tcg_ctx, left, a->vm, 0, MO_64);
        tcg_gen_extract2_i64(tcg_ctx, dest, right, left, a->imm * 8);
        write_neon_element64(tcg_ctx, dest, a->vd, 0, MO_64);
        ...
        if (a->imm < 8) {
            read_neon_element64(tcg_ctx, right, a->vn, 0, MO_64);
            read_neon_element64(tcg_ctx, middle, a->vn, 1, MO_64);
            tcg_gen_extract2_i64(tcg_ctx, destright, right, middle, a->imm * 8);
            read_neon_element64(tcg_ctx, left, a->vm, 0, MO_64);
            tcg_gen_extract2_i64(tcg_ctx, destleft, middle, left, a->imm * 8);
        } else {
            read_neon_element64(tcg_ctx, right, a->vn, 1, MO_64);
            read_neon_element64(tcg_ctx, middle, a->vm, 0, MO_64);
            tcg_gen_extract2_i64(tcg_ctx, destright, right, middle, (a->imm - 8) * 8);
            read_neon_element64(tcg_ctx, left, a->vm, 1, MO_64);
            tcg_gen_extract2_i64(tcg_ctx, destleft, middle, left, (a->imm - 8) * 8);
        }

        write_neon_element64(tcg_ctx, destright, a->vd, 0, MO_64);
        write_neon_element64(tcg_ctx, destleft, a->vd, 1, MO_64);
...
static bool trans_VTBL(DisasContext *s, arg_VTBL *a)
{
    ...
    if (a->op) {
        read_neon_element64(tcg_ctx, def, a->vd, 0, MO_64);
    } else {
        tcg_gen_movi_i64(tcg_ctx, def, 0);
    }
    val = tcg_temp_new_i64(tcg_ctx);
    read_neon_element64(tcg_ctx, val, a->vm, 0, MO_64);

    gen_helper_neon_tbl(tcg_ctx, val, tcg_ctx->cpu_env, desc, val, def);
    write_neon_element64(tcg_ctx, val, a->vd, 0, MO_64);
...
static bool trans_VREV64(DisasContext *s, arg_VREV64 *a)
{
    ...
    for (pass = 0; pass < (a->q ? 2 : 1); pass++) {
        for (half = 0; half < 2; half++) {
            read_neon_element32(tcg_ctx, tmp[half], a->vm, pass * 2 + half, MO_32);
            switch (a->size) {
            case 0:
        ...
        write_neon_element32(tcg_ctx, tmp[1], a->vd, pass * 2, MO_32);
        write_neon_element32(tcg_ctx, tmp[0], a->vd, pass * 2 + 1, MO_32);
    }
                              ...
                              NeonGenTwo64OpFn *accfn)
{
    ...
    for (pass = 0; pass < a->q + 1; pass++) {
        ...
        tmp = tcg_temp_new_i32(tcg_ctx);
        read_neon_element32(tcg_ctx, tmp, a->vm, pass * 2, MO_32);
        widenfn(tcg_ctx, rm0_64, tmp);
        read_neon_element32(tcg_ctx, tmp, a->vm, pass * 2 + 1, MO_32);
        ...
        if (accfn) {
            ...
            read_neon_element64(tcg_ctx, tmp64, a->vd, pass, MO_64);
            accfn(tcg_ctx, rd_64, tmp64, rd_64);
            tcg_temp_free_i64(tcg_ctx, tmp64);
        }
        write_neon_element64(tcg_ctx, rd_64, a->vd, pass, MO_64);
                     ...
                     NeonGenNarrowEnvFn *narrowfn)
{
    ...
    read_neon_element64(tcg_ctx, rm, a->vm, 0, MO_64);
    narrowfn(tcg_ctx, rd0, tcg_ctx->cpu_env, rm);
    read_neon_element64(tcg_ctx, rm, a->vm, 1, MO_64);
...
static bool trans_VSHLL(DisasContext *s, arg_2misc *a)
{
    ...
    write_neon_element64(tcg_ctx, rd, a->vd, 0, MO_64);
    widenfn(tcg_ctx, rd, rm1);
    tcg_gen_shli_i64(tcg_ctx, rd, rd, 8 << a->size);
    write_neon_element64(tcg_ctx, rd, a->vd, 1, MO_64);
...
static bool trans_VCVT_F16_F32(DisasContext *s, arg_2misc *a)
{
    ...
    read_neon_element32(tcg_ctx, tmp, a->vm, 2, MO_32);
    gen_helper_vfp_fcvt_f32_to_f16(tcg_ctx, tmp, tmp, fpst, ahp);
    tmp3 = tcg_temp_new_i32(tcg_ctx);
    read_neon_element32(tcg_ctx, tmp3, a->vm, 3, MO_32);
    write_neon_element32(tcg_ctx, tmp2, a->vd, 0, MO_32);
    tcg_temp_free_i32(tcg_ctx, tmp2);
...
static void gen_VABS_F(TCGContext *tcg_ctx, unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs,
                       uint32_t oprsz, uint32_t maxsz)
{
    tcg_gen_gvec_andi(tcg_ctx, vece, rd_ofs, rm_ofs,
...
static void gen_VNEG_F(TCGContext *tcg_ctx, unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs,
                       uint32_t oprsz, uint32_t maxsz)
{
    tcg_gen_gvec_xori(tcg_ctx, vece, rd_ofs, rm_ofs,
...
#define DO_2MISC_FP_VEC(INSN, HFUNC, SFUNC)                             \
    static void gen_##INSN(TCGContext *tcg_ctx, unsigned vece, uint32_t rd_ofs,              \
                           uint32_t rm_ofs,                             \
                           uint32_t oprsz, uint32_t maxsz)              \
    {                                                                   \
        ...
        TCGv_ptr fpst;                                                  \
        fpst = fpstatus_ptr(tcg_ctx, vece == MO_16 ? FPST_STD_F16 : FPST_STD);   \
        tcg_gen_gvec_2_ptr(tcg_ctx, rd_ofs, rm_ofs, fpst, oprsz, maxsz, 0,       \
                           fns[vece]);                                  \
        tcg_temp_free_ptr(tcg_ctx, fpst);                                        \
    }                                                                   \
    static bool trans_##INSN(DisasContext *s, arg_2misc *a)             \
...
#define DO_VEC_RMODE(INSN, RMODE, OP)                                   \
    ...
    {                                                                   \
        ...
        TCGv_ptr fpst;                                                  \
        fpst = fpstatus_ptr(tcg_ctx, vece == 1 ? FPST_STD_F16 : FPST_STD);       \
        tcg_gen_gvec_2_ptr(tcg_ctx, rd_ofs, rm_ofs, fpst, oprsz, maxsz,          \
...
static bool trans_VSWP(DisasContext *s, arg_2misc *a)
{
    ...
    rd = tcg_temp_new_i64(tcg_ctx);
    for (pass = 0; pass < (a->q ? 2 : 1); pass++) {
        read_neon_element64(tcg_ctx, rm, a->vm, pass, MO_64);
        read_neon_element64(tcg_ctx, rd, a->vd, pass, MO_64);
        write_neon_element64(tcg_ctx, rm, a->vd, pass, MO_64);
        write_neon_element64(tcg_ctx, rd, a->vm, pass, MO_64);
    }
    tcg_temp_free_i64(tcg_ctx, rm);
    tcg_temp_free_i64(tcg_ctx, rd);
...
static bool trans_VTRN(DisasContext *s, arg_2misc *a)
{
    ...
    if (a->size == MO_32) {
        for (pass = 0; pass < (a->q ? 4 : 2); pass += 2) {
            read_neon_element32(tcg_ctx, tmp, a->vm, pass, MO_32);
            read_neon_element32(tcg_ctx, tmp2, a->vd, pass + 1, MO_32);
            write_neon_element32(tcg_ctx, tmp2, a->vm, pass, MO_32);
            write_neon_element32(tcg_ctx, tmp, a->vd, pass + 1, MO_32);
        }
    } else {
        for (pass = 0; pass < (a->q ? 4 : 2); pass++) {
            read_neon_element32(tcg_ctx, tmp, a->vm, pass, MO_32);
            read_neon_element32(tcg_ctx, tmp2, a->vd, pass, MO_32);
            if (a->size == MO_8) {
